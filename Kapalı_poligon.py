...
class deneme2():
    ...
    def main(self):
        ...
        self.win = tk.Tk()
        self.win.title("Kapalı Poligon Hesabı")
    ...
    def create_labels(self):
        ...
        H2 = ttk.Label(self.scrollable_frame, text="NNokta Adı/ Numarası")
    ...
    def create_table(self, point_count, nirengi_count):
        ...
        for i in range(2, self.point_count + self.nirengi_count - 1):
        ...
        self.arr[self.point_count + self.nirengi_count -1][self.DELTA_X_DIFFERENCE_INDEX].config(state='disabled')
        self.arr[self.point_count + self.nirengi_count - 1][self.AZIMUTH_ANGLE_INDEX].config(state='disabled')
        self.arr[self.point_count + self.nirengi_count - 1][self.ACIKLIK_ANGLE_INDEX].config(state='disabled')
        self.arr[self.point_count + self.nirengi_count - 1][self.AZIMUTH_ANGLE_DIFFERENCE_INDEX].config(state='disabled')
        self.arr[self.point_count + self.nirengi_count -1][self.DELTA_X_INDEX].config(state='disabled')
        self.arr[self.point_count + self.nirengi_count -1][self.DELTA_Y_INDEX].config(state='disabled')
        self.arr[self.point_count + self.nirengi_count -1][self.DELTA_Y_DIFFERENCE_INDEX].config(state='disabled')
        self.arr[self.point_count + self.nirengi_count -1][self.HORIZONTAL_EDGE_INDEX].config(state='disabled')
    ...
    def get_values(self):
        ...
        self.total_row = self.point_count + self.nirengi_count - 1
        #self.contol()
        difference = self.find_difference()
        value_to_add_rows = self.convert_number_to_float_5_digit(difference / (self.point_count + 1))
    ...
    def create_excel(self):
        ...
        worksheet.write(self.nirengi_count + self.point_count + 1, 6,
                        'S = {}'.format(self.convert_number_to_float_3_digit(self.total_s)), cell_format3)
        worksheet.write(self.nirengi_count + self.point_count + 1, 11,
                        'Δy = {}'.format(self.convert_number_to_float_3_digit(self.diff_y)), cell_format3)
        worksheet.write(self.nirengi_count + self.point_count + 1, 12,
                        'Δx = {}'.format(self.convert_number_to_float_3_digit(self.diff_x)), cell_format3)
        worksheet.write(self.nirengi_count + self.point_count + 1, 10,
                        '∑Δx = {}'.format(self.convert_number_to_float_3_digit(self.total_delta_x - self.diff_x)),
                        cell_format3)
        worksheet.write(self.nirengi_count + self.point_count + 1, 8,
                        '∑Δy = {}'.format(self.convert_number_to_float_3_digit(self.total_delta_y - self.diff_y)),
                        cell_format3)
        worksheet.write(self.nirengi_count + self.point_count + 1, 4,
                        '∑⦬ = {}'.format(self.convert_number_to_float_5_digit(self.last_sum)), cell_format3)

        workbook.close()
        os.startfile('write_list.xlsx')
    ...
    def create_plot(self):
        ...
        combine_x_y = []
        name_of_coordinate = []
        for i in range(1, self.point_count + self.nirengi_count):
        ...
        for i, type in enumerate(name_of_coordinate):
            ...
            x = x_index_value[i]
            plt.scatter(y, x)
            plt.text(y + 1, x + 1, type, fontsize=15)
        ...
        plt.axis([float(self.arr[1][self.Y_CORDINATE_INDEX].get()) - 10,
                  float(self.arr[self.total_row][self.Y_CORDINATE_INDEX].get()) + 150,
                  float(self.arr[1][self.X_CORDINATE_INDEX].get()) - 100,
                  float(self.arr[self.total_row][self.X_CORDINATE_INDEX].get()) + 150])
        plt.show()

        '''point_coordinate=np.array(combine_x_y).T.tolist()
        x,y=point_coordinate.T
        plt.scatter(x,y)
        plt.show()'''
    def contol(self):
            list1=[self.ACIKLIK_ANGLE_INDEX,self.ACIKLIK_ANGLE_INDEX,self.Y_CORDINATE_INDEX,self.Y_CORDINATE_INDEX,self.Y_CORDINATE_INDEX,self.X_CORDINATE_INDEX,self.X_CORDINATE_INDEX,self.X_CORDINATE_INDEX,]
            list2=[0,self.point_count + self.nirengi_count-1,0,1,self.point_count + self.nirengi_count-1,0,1,self.point_count + self.nirengi_count-1]
    ...
    def contol_delta_xy(self, total_row):
        SS = ((float(self.total_delta_x)) * (
                    float(self.total_delta_x) + (float(self.total_delta_y)) * float(self.total_delta_y)))
        S = math.sqrt(SS)
        ...
        if self.fQ > self.FQ or self.fL > self.FL:
            ...
            messagebox.showinfo("Information",
                                "fQ = {} FQ ={} \n fL={} FL={} ".format(self.convert_number_to_float_3_digit(self.fQ),
                                                                        self.convert_number_to_float_3_digit(self.FQ),
                                                                        self.convert_number_to_float_3_digit(self.fL),
                                                                        self.convert_number_to_float_3_digit(self.FL)))
        else:
            self.calculate_new_y_cordinates(self.total_row)
            self.calculate_new_x_cordinates(self.total_row)
    ...
    def calculate_deltas(self, total_row):
        ...
        self.diff_x = float(self.arr[self.total_row][self.X_CORDINATE_INDEX].get()) - float(
            self.arr[1][self.X_CORDINATE_INDEX].get())
        self.diff_y = float(self.arr[self.total_row][self.Y_CORDINATE_INDEX].get()) - float(
            self.arr[1][self.Y_CORDINATE_INDEX].get())
    ...
    def add_error_to_delta_values(self, err_y, err_x, total_row):
        ...
        for i in range(1, self.total_row):
            new_delta_y_value = self.convert_number_to_float_3_digit(
                float(self.arr[i][self.DELTA_Y_INDEX].get())) - self.convert_number_to_float_3_digit(
                ((err_y / self.total_s) * float(self.arr[i][self.HORIZONTAL_EDGE_INDEX].get())))
            new_delta_x_value = self.convert_number_to_float_3_digit(
                float(self.arr[i][self.DELTA_X_INDEX].get()) - self.convert_number_to_float_3_digit(
                    ((err_x / self.total_s) * float(self.arr[i][self.HORIZONTAL_EDGE_INDEX].get()))))

            self.clear_old_value(self.arr[i][self.DELTA_Y_INDEX])
            self.clear_old_value(self.arr[i][self.DELTA_X_INDEX])

            self.x_hata = self.convert_number_to_float_3_digit((self.convert_number_to_float_3_digit(
                ((err_x / self.total_s) * float(self.arr[i][self.HORIZONTAL_EDGE_INDEX].get())))))
            self.y_hata = self.convert_number_to_float_3_digit((self.convert_number_to_float_3_digit(
                ((err_y / self.total_s) * float(self.arr[i][self.HORIZONTAL_EDGE_INDEX].get())))))
    ...
    def calculate_delta_y(self, i):
        delta_y = math.sin(math.radians(float(self.arr[i][self.ACIKLIK_ANGLE_INDEX].get()) / 400 * 360)) * float(
            self.arr[i][self.HORIZONTAL_EDGE_INDEX].get())
    ...
    def calculate_delta_x(self, i):
        delta_x = math.cos(math.radians(float(self.arr[i][self.ACIKLIK_ANGLE_INDEX].get()) / 400 * 360)) * float(
            self.arr[i][self.HORIZONTAL_EDGE_INDEX].get())
    ...
    def calculate_azimuth_angle(self):
        for i in range(0, self.point_count):
            ...
            self.arr[i + 1][self.ACIKLIK_ANGLE_INDEX].insert(0, str(self.convert_number_to_float_4_digit(processed_value)))

    def apply_error_to_all_values(self, difference, total_row):
        for i in range(1, self.point_count + 2):
            new_value = float(self.arr[i][self.AZIMUTH_ANGLE_INDEX].get()) + difference
            ...
            self.arr[i][self.AZIMUTH_ANGLE_DIFFERENCE_INDEX].insert(0, difference)

    def find_difference(self):
        total_sum = 0
        for i in range(1,self.point_count+2 ):
            total_sum += float(self.arr[i ][self.AZIMUTH_ANGLE_INDEX].get())
        difference = total_sum -((self.point_count+3)*200)
        self.last_sum=difference
        if (self.last_sum < 0):
    ...
    def convert_entry_to_float_4_digit(self, entry):
        return float("{0:.4f}".format(float(entry.get())))

    def convert_number_to_float_4_digit(self, entry):
        return float("{0:.4f}".format(float(entry)))
