...
class nivo_bağlı():
    labels = ['Nokta No', 'Geri Mesafe', 'İleri Mesafe', 'Geri Okuma', 'Orta Okuma', 'İleri Okuma',
              'Yükseklik Farkı', 'Değişim', 'Gözleme Düzlemi Kotu', 'Nokta Yükseklikleri']
    valueOfTemp = 0
    ...
    def main(self):
        ...
        self.GERİ_MESAFE = 1
        self.İLERİ_MESAFE = 2
        self.GERİ_OKUMA_INDEX = 3
        self.ORTA_OKUMA_INDEX = 4
        self.İLERİ_OKUMA_INDEX = 5
        self.YÜKSEKLİK_FARKI = 6
        self.DELTA_H_DEĞİŞİM = 7
        self.GDK = 8
        self.NOKTA_YÜKSEKLİKLERİ = 9

        self.win = tk.Tk()
        self.win.title('Bağlı Nivelman Hesabı')
        self.win.geometry("1380x350")
        self.win.resizable(0, 0)

        self.h_known = 2
        self.nokta_sayısı = self.valueOfTemp
        self.frame = Frame(self.win, width=1352, height=600, background='#EBEBEB')
        self.frame.grid()
        self.frame.rowconfigure(0, weight=1)
        self.frame.columnconfigure(0, weight=1)

        self.canvas = tk.Canvas(self.frame, height=332, width=1358, background='#EBEBEB')
    ...
    def create_labels(self):
        H1 = ttk.Label(self.scrollable_frame, text="Nokta Adı/ Numarası")
        H2 = ttk.Label(self.scrollable_frame, text="Geri Mesafe (m)")
        H3 = ttk.Label(self.scrollable_frame, text="İleri Mesafe (m)")
        H4 = ttk.Label(self.scrollable_frame, text="Geri Okuma (mm)")
        H5 = ttk.Label(self.scrollable_frame, text="Orta Okuma (mm)")
        H6 = ttk.Label(self.scrollable_frame, text="İleri Okuma (mm)")
        H7 = ttk.Label(self.scrollable_frame, text="Yükseklik Farkı (mm)")
        H8 = ttk.Label(self.scrollable_frame, text="Değişim (mm)")
        H9 = ttk.Label(self.scrollable_frame, text="Gözleme Düzlemi Kotu (m)")
        H10 = ttk.Label(self.scrollable_frame, text="Nokta Yükseklikleri (m)")
        ...
        H9.grid(row=0, column=8, padx=10, pady=10)
        H10.grid(row=0, column=9, padx=10, pady=10)

    def create_table(self, nokta_sayısı, h_known):
        cols = 10
        nokta_sayısı += h_known
        rows = nokta_sayısı
        ...
        self.arr[0][self.İLERİ_MESAFE].config(state='disabled')
        ...
        self.arr[0][self.DELTA_H_DEĞİŞİM].config(state='disabled')
        self.arr[rows-1][self.GERİ_OKUMA_INDEX].config(state='disabled')
        self.arr[rows-1][self.GDK].config(state='disabled')
        self.arr[rows-1][self.GERİ_MESAFE].config(state='disabled')
    ...
    def create_excel(self):
        ...
        workbook = xlsxwriter.Workbook('bağlı_nivelman_çıktı.xlsx')
        ...
        worksheet.set_column(0, len(self.labels), 20)
        ...
        for i in range(1, self.nokta_sayısı + self.h_known):
            ...
            değişim.insert(i, self.arr[i][self.DELTA_H_DEĞİŞİM].get())
            ileri_mesafe.insert(i, float(self.arr[i][self.İLERİ_MESAFE].get()))

        for i in range(0, self.nokta_sayısı + self.h_known-1):
             geri_okuma.insert(i, float(self.arr[i][self.GERİ_OKUMA_INDEX].get()))
             gdk.insert(i, float(self.arr[i][self.GDK].get()))
             geri_mesafe.insert(i, float(self.arr[i][self.GERİ_MESAFE].get()))

        for i in range(0, self.nokta_sayısı + self.h_known):
        ...
        for row_num, data in enumerate(geri_mesafe):
            worksheet.write(row_num+1, 2, data, cell_format2)
        for row_num, data in enumerate(ileri_mesafe):
        ...
        for row_num, data in enumerate(değişim):
            worksheet.write(row_num+2, 8, data, cell_format2)
        for row_num, data in enumerate(gdk):
            worksheet.write(row_num + 1, 9, data,cell_format2)
        for row_num, data in enumerate(nokta_yükseklikleri):
            worksheet.write(row_num + 1, 10, data,cell_format2)
        workbook.close()
        os.startfile('bağlı_nivelman_hesabı.xlsx')
    ...
    def hata_kontrol(self):
        ...
        for i in range(0, self.total_row):
            setup_diff.insert(i, self.arr[i][self.GERİ_MESAFE].get())
            self.available_1 = 0
            

            if len(self.arr[i][self.GERİ_OKUMA_INDEX].get()) != 0:
                self.available_1 = float(self.arr[i][self.GERİ_OKUMA_INDEX].get())
                geri_okuma.insert(i, float(self.arr[i][self.GERİ_OKUMA_INDEX].get()))
            
        for i in range(1, self.total_row+1):
            setup_diff.insert(i, self.arr[i][self.İLERİ_MESAFE].get())
            self.available_2 = 0

            if len(self.arr[i][self.İLERİ_OKUMA_INDEX].get()) != 0:
                self.available_2 = float(self.arr[i][self.İLERİ_OKUMA_INDEX].get())
                ileri_okuma.insert(i, float(self.arr[i][self.İLERİ_OKUMA_INDEX].get()))
                
        self.tot_geri = sum(Decimal(i) for i in geri_okuma)
        self.tot_ileri = sum(Decimal(i) for i in ileri_okuma)   
        self.okuma_fark = float(self.tot_geri - self.tot_ileri)
        
        self.tot_mesafe = float(sum(Decimal(i) for i in setup_diff))

        self.f_h = self.okuma_fark - (float(self.arr[self.total_row][self.NOKTA_YÜKSEKLİKLERİ].get())-float(self.arr[0][self.NOKTA_YÜKSEKLİKLERİ].get()))*1000
        self.L = float(self.tot_mesafe/1000)
        self.f_h_max = (0.02 * self.L) + (0.00030 * (float(self.arr[self.total_row][self.NOKTA_YÜKSEKLİKLERİ].get())-float(self.arr[0][self.NOKTA_YÜKSEKLİKLERİ].get()))*1000)


        if self.f_h < self.f_h_max:
            messagebox.showinfo("UYARI", "Ölçümünüz doğru kapanmıştır.")
        else:
            messagebox.showerror("UYARI", "Ölçüm Hoşgörü Sınırının Üstünde \n Hata Miktarı: {} \n Hoşgörü Miktarı: {}".format(self.convert_number_to_float_3_digit(self.f_h), self.convert_number_to_float_3_digit(self.f_h_max)))
    ...
    def dağıtım(self):
        ...
        for i in range(0, self.total_row):
            if len(self.arr[i][self.GERİ_OKUMA_INDEX].get()) != 0:
                self.available_3 = float(self.arr[i][self.GERİ_OKUMA_INDEX].get())
                geri.insert(i, float(self.arr[i][self.GERİ_OKUMA_INDEX].get()))
        
        
        self.hata_payı = float(self.f_h / len(geri))
        
        for i in range(0, self.total_row-1):
            if len(self.arr[i][self.GERİ_OKUMA_INDEX].get()) != 0:
                self.arr[i+1][self.DELTA_H_DEĞİŞİM].insert(i, self.convert_number_to_float_5_digit(self.hata_payı))
                new_value = float(self.arr[i+1][self.YÜKSEKLİK_FARKI].get()) + (self.hata_payı)
                self.clear_old_value(self.arr[i + 1][self.YÜKSEKLİK_FARKI])
                self.arr[i + 1][self.YÜKSEKLİK_FARKI].insert(i, self.convert_number_to_float_3_digit(new_value))

        for i in range(0,self.total_row):
            self.clear_old_value(self.arr[i+1][self.NOKTA_YÜKSEKLİKLERİ])
            self.arr[i+1][self.NOKTA_YÜKSEKLİKLERİ].insert(i, self.convert_number_to_float_3_digit(float(self.arr[i + 1][self.YÜKSEKLİK_FARKI].get()) / 1000 + float(self.arr[i][self.NOKTA_YÜKSEKLİKLERİ].get())))

    def kot_hesabı(self):
        for i in range(0, self.total_row - 1):
            self.nokta_kotu = float(self.arr[i][self.NOKTA_YÜKSEKLİKLERİ].get()) + float(
                self.arr[i + 1][self.YÜKSEKLİK_FARKI].get()) / 1000
    ...
    def GDK_hesabı(self):
        for i in range(0, self.total_row):
            if len(self.arr[i][self.GERİ_OKUMA_INDEX].get()) == 0:
                ...
                self.gdk = float(self.arr[i][self.NOKTA_YÜKSEKLİKLERİ].get()) + float(
                    self.arr[i][self.GERİ_OKUMA_INDEX].get())
                self.arr[i][self.GDK].insert(i, self.convert_number_to_float_3_digit(self.gdk))
