...
class map2:
    labels = ['Nokta No', 'Y Koordinat(WGS84)', 'X Koordinat(WGS84)']
    valueOfTemp = 0
    ...
    def create_labels(self):
        ...
        H2 = ttk.Label(self.scrollable_frame, text='Y Koordinat(WGS84)')
        H3 = ttk.Label(self.scrollable_frame, text='X Koordinat(WGS84)')
    ...
    def create_table(self, nokta_sayısı):
            ...
            self.calculate_button = ttk.Button(self.scrollable_frame, text="Göster", command=self.get_values)
    ...
    def map2(self, nokta_sayısı):
        ...
        name = []
        for i in range(1, self.nokta_sayısı + 1):
            y_coord.insert(i, self.arr[i][self.Longitude].get())
            x_coord.insert(i, self.arr[i][self.Latitude].get())
